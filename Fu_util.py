...
def fu_melds(group,isHand,selfWind,dealerWind):
    ...
    if len(group)==2:
        if group[0] in tile_constants.DragonTiles or group[0] == selfWind or group[0] == dealerWind: 
    ...
    if(group[0] in tile_constants.WordTiles):  fu*= 2             # if the group is honor tiles(word tiles), multiply by 2 again
    return fu
